...
try {
  ...
  const result=await page.evaluate(async({width,height,runs,profile,gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,captureBoundaries})=>{
    ...
    const engine=await NeuralRenderer.create(model,{gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode}),runtime=engine.runtime,measurements=[];
    ...
    try {
      for(let run=0;run<runs;run++){
        ...
        const output=await engine.run({width,height,proxy,profile,capture:captureBoundaries?async(name,data)=>{boundaries[name]=await hash(data);}:undefined});
        const elapsedMs=performance.now()-start;
        if(!output.output.every(Number.isFinite)||!output.head.every(Number.isFinite))throw Error('Non-finite benchmark output.');
        const measurement={run:run+1,boundaries,workingBuffers:output.workingBuffers,workspaceBytes:engine.workspaceBytes,profile:output.profile,elapsedMs,decodeMs,waitMs,waits,weightCacheBytes:engine.weightCacheBytes,bindGroupsCreated:runtime.stats.bindGroupsCreated-before.bindGroupsCreated,dataBytesUploaded:runtime.stats.dataBytesUploaded-before.dataBytesUploaded,submissions:runtime.stats.submissions-before.submissions,headSha256:await hash(output.head),outputSha256:await hash(output.output)};
      ...
      return {width,height,configuration:{profile,gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,captureBoundaries},adapter:runtime.describe(),measurements};
    }finally{engine.dispose();}
  },{width,height,runs,executionMode:process.env.NR_EXECUTION||'prepared',activationStorage:process.env.NR_ACTIVATIONS||'packed',profile:process.env.NR_PROFILE==='1',captureBoundaries:process.env.NR_CAPTURE==='1',gemmMode:process.env.NR_GEMM||'auto',attentionMode:process.env.NR_ATTENTION||'tiled',workspaceCacheBytes:Number(process.env.NR_WORKSPACE_MIB??256)*1024*1024});
  if(errors.length)throw Error(errors.join('; '));
  result.browser=browser.version();
  if(process.env.NR_COMPARE){const baseline=JSON.parse(await readFile(process.env.NR_COMPARE,'utf8'));if(baseline.width!==width||baseline.height!==height)throw Error('Baseline dimensions differ.');for(const run of result.measurements)if(run.headSha256!==baseline.measurements[0].headSha256||run.outputSha256!==baseline.measurements[0].outputSha256)throw Error('Output differs from baseline.');if(baseline.measurements[0].boundaries){for(const run of result.measurements)if(JSON.stringify(run.boundaries)!==JSON.stringify(baseline.measurements[0].boundaries))throw Error('Intermediate boundaries differ from baseline.');}result.matchesBaseline=true;}

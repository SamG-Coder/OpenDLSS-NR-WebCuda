...
try {
  ...
  const result=await page.evaluate(async({width,height,runs,profile,gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,graphBatchSize,maxInFlightBatches,cacheNoise,readHead,captureBoundaries})=>{
    ...
    const engine=await NeuralRenderer.create(model,{gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,graphBatchSize,maxInFlightBatches,cacheNoise}),runtime=engine.runtime,measurements=[];
    ...
    try {
      ...
      return {width,height,configuration:{profile,gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,graphBatchSize,maxInFlightBatches,cacheNoise,readHead,captureBoundaries},adapter:runtime.describe(),measurements};
    }finally{engine.dispose();}
  },{width,height,runs,maxInFlightBatches:Number(process.env.NR_INFLIGHT??4),cacheNoise:process.env.NR_NOISE_CACHE!=='0',readHead:process.env.NR_READ_HEAD!=='0',graphBatchSize:Number(process.env.NR_BATCH??32),executionMode:process.env.NR_EXECUTION||'prepared',activationStorage:process.env.NR_ACTIVATIONS||'packed',profile:process.env.NR_PROFILE==='1',captureBoundaries:process.env.NR_CAPTURE==='1',gemmMode:process.env.NR_GEMM||'auto',attentionMode:process.env.NR_ATTENTION||'fused',workspaceCacheBytes:Number(process.env.NR_WORKSPACE_MIB??256)*1024*1024});

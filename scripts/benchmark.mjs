...
try {
  ...
  const result=await page.evaluate(async({width,height,runs,profile,gemmMode,attentionMode,workspaceCacheBytes,captureBoundaries})=>{
    ...
    const engine=await NeuralRenderer.create(model,{gemmMode,attentionMode,workspaceCacheBytes}),runtime=engine.runtime,measurements=[];
    ...
    try {
      for(let run=0;run<runs;run++){
        ...
        const measurement={run:run+1,boundaries,workingBuffers:output.workingBuffers,workspaceBytes:engine.workspaceBytes,profile:output.profile,elapsedMs,decodeMs,waitMs,waits,weightCacheBytes:engine.weightCacheBytes,dataBytesUploaded:runtime.stats.dataBytesUploaded-before.dataBytesUploaded,submissions:runtime.stats.submissions-before.submissions,headSha256:await hash(output.head),outputSha256:await hash(output.output)};
      ...
      return {width,height,configuration:{profile,gemmMode,attentionMode,workspaceCacheBytes,captureBoundaries},adapter:runtime.describe(),measurements};
    }finally{engine.dispose();}
  },{width,height,runs,profile:process.env.NR_PROFILE==='1',captureBoundaries:process.env.NR_CAPTURE==='1',gemmMode:process.env.NR_GEMM||'auto',attentionMode:process.env.NR_ATTENTION||'tiled',workspaceCacheBytes:Number(process.env.NR_WORKSPACE_MIB??256)*1024*1024});

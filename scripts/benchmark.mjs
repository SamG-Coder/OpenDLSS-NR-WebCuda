...
try {
  ...
  const result=await page.evaluate(async({width,height,runs,profile,gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,graphBatchSize,captureBoundaries})=>{
    ...
    const engine=await NeuralRenderer.create(model,{gemmMode,attentionMode,workspaceCacheBytes,activationStorage,executionMode,graphBatchSize}),runtime=engine.runtime,measurements=[];

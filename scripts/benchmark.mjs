...
try {
  ...
  const result=await page.evaluate(async({width,height,runs,profile,gemmMode,workspaceCacheBytes})=>{
    ...
    const engine=await NeuralRenderer.create(model,{gemmMode,workspaceCacheBytes}),runtime=engine.runtime,measurements=[];
    ...
    try {
      for(let run=0;run<runs;run++){
        ...
        measurements.push(measurement);console.log(JSON.stringify({...measurement,profile:measurement.profile?{supported:measurement.profile.supported,byKernel:measurement.profile.byKernel}:undefined}));
      }
      return {width,height,adapter:runtime.describe(),measurements};
    }finally{engine.dispose();}
  },{width,height,runs,profile:process.env.NR_PROFILE==='1',gemmMode:process.env.NR_GEMM||'tiled',workspaceCacheBytes:Number(process.env.NR_WORKSPACE_MIB??256)*1024*1024});

...
try {
  ...
  const configuration={runs,previousEngine:process.env.NR_PREVIOUS_ENGINE||null,gemmTile:process.env.NR_TILE||'auto',previousGenerated,enhanced,normalizeAttention,wideGemm,nativeHalf:wideGemm||process.env.NR_HALF==='1',specialization:process.env.NR_SPECIALIZE==='1'};
  const results=await page.evaluate(async({runs,previousEngine,gemmTile,specialization,nativeHalf,wideGemm,normalizeAttention,enhanced,previousGenerated})=>{

...
try {
  ...
  const png=await page.evaluate(()=>{const c=document.createElement('canvas');c.width=c.height=33;const x=c.getContext('2d');for(let y=0;y<33;y++)for(let a=0;a<33;a++){x.fillStyle=`rgb(${a*7},${y*7},100)`;x.fillRect(a,y,1,1);}return c.toDataURL().split(',')[1];});
  await page.locator('#image').setInputFiles({name:'gradient.png',mimeType:'image/png',buffer:Buffer.from(png,'base64')});
  await page.waitForFunction(()=>!document.querySelector('#run').disabled);
  await page.evaluate(()=>{new MutationObserver(()=>{const s=document.querySelector('#status').textContent,m=s.match(/· (\d+) \/ /);if(m&&Number(m[1])%50===0)console.log(s);}).observe(document.querySelector('#status'),{childList:true});});
  ...
  const result=await page.evaluate(()=>{
    const status=document.querySelector('#status').textContent,c=document.querySelector('#output'),p=c.getContext('2d').getImageData(0,0,c.width,c.height).data,src=document.querySelector('#input').getContext('2d').getImageData(0,0,33,33).data;
  ...
  if(errors.length||!result.status.startsWith('Completed in ')||!result.downloadReady||result.width!==33||result.height!==33||!result.differentChannels||result.min===result.max)throw Error('Real-model rendering failed: '+result.status+' '+errors.join('; '));

  ...
  const results=await page.evaluate(async()=>{
    ...
    for(const [width,height] of [[512,512],[1280,720],[1920,1080]]){
      ...
      try {
        ...
        const upStart=performance.now();upstream=await Network.create({weights:'/comparison-model',width,height});const upstreamSetupMs=performance.now()-upStart;
        ...
        const result={width,height,paddedWidth:g.fullWidth,paddedHeight:g.fullHeight,inputSha256,modelLoadMs,oursSetupMs,upstreamSetupMs,cold,samples,headHashes:{ours:await hash(heads.ours),upstream:await hash(heads.upstream)},mismatchCount,maxAbsError,headElements:heads.ours.length,dispatches:{ours:ours.graphCache.graph.ops.length,upstream:upstream.recorder.dispatchCount},memory:{upstreamActivations:upstream.tensors.total,upstreamWeights:upstream.model.bytesUploaded,oursPlan:ours.plan?.bytes,oursWeights:ours.weightCacheBytes},adapter:{ours:runtime.describe(),upstream:{vendor:upstream.adapterInfo.vendor,architecture:upstream.adapterInfo.architecture,features:[...upstream.device.features],workgroupStorage:upstream.device.limits.maxComputeWorkgroupStorageSize}}};
        results.push(result);console.log('COMPARE '+JSON.stringify(result));
      }finally{upstream?.destroy();ours?.dispose();}
    }
    return results;
  });
  assert.deepEqual(errors,[]);
  await writeFile(process.env.NR_REPORT||'reports/upstream-comparison.json',JSON.stringify({oursRevision,upstreamRevision:revision,browser:browser.version(),scope:'Identical precomputed features: upload + graph execution + float32 head readback; no preprocessing, composition, or UI.',results},null,2));

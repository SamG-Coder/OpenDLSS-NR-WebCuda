...
try {
  ...
  const results=await page.evaluate(async()=>{
    ...
    const reference=await NeuralRenderer.create(model,{activationStorage:'float',executionMode:'streamed',attentionMode:'tiled',cacheNoise:false,maxInFlightBatches:1,specializeGemm:false}),prepared=await NeuralRenderer.create(model,{nativeHalf:true});

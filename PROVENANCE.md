...
- Compiler/runtime: `https://github.com/SamG-Coder/cuda-webshader`, revision `f0f3699`. The bit-reinterpretation and adapter-buffer-limit patches are now upstream (`390f6c1`, `166056c`), alongside native local/shared half support (`5aa80e3`), 32-bit CUDA `__clz` support (`b8e8cd0`), opt-in adapter workgroup storage limits (`7cc771c`), bounded loop unrolling/component-wise vector lowering (`f82d847`), vector extrema reductions (`db1e3d2`), and native half storage-buffer parameters with binary16 CPU-oracle access (`f0f3699`). Historical patches remain in `patches/`.

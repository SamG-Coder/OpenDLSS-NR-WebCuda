...
- Compiler/runtime: `https://github.com/SamG-Coder/cuda-webshader`, revision `f82d847`. The bit-reinterpretation and adapter-buffer-limit patches are now upstream (`390f6c1`, `166056c`), alongside native local/shared half support (`5aa80e3`) and 32-bit CUDA `__clz` support (`b8e8cd0`) and opt-in adapter workgroup storage limits (`7cc771c`) and bounded loop unrolling/component-wise vector lowering (`f82d847`). Historical patches remain in `patches/`.

...
Native local/shared `__half` and `__half2` support was introduced in WebCuda commit `5aa80e3`, pushed to its own repository. That upstream history also contains the two earlier patches as `390f6c1` and `166056c`. Validation of that change: 768 host tests, existing kernel compilation, and 41,656 exact GPU half-product pairs. Commit [`f0f3699`](https://github.com/SamG-Coder/cuda-webshader/commit/f0f3699b498cfe6fe5419e072a4f4e2faa63b781) extends this interface to native half storage buffers for precomputed operands, with 782 host tests and hardware half/half2 storage read/write checks passing. Both compiler and CPU oracle are synced here; by-value half kernel parameters remain unsupported.
...
CUDA `__clz` now replaces the fixed-point rounding bit-search loop. WebCuda commit `b8e8cd0` is pushed upstream and synced here. The [full shader and execution comparison](reports/full-shader-comparison.md) includes per-family GPU timestamps, actual upstream shader/pipeline capture, matching network-head checks, and remaining priorities. Set `NR_PROFILE=1` when running `npm run benchmark:upstream` to capture three profiled runs after the normal six-sample timing comparison; raw upstream shader captures remain ignored and local. `NR_AUDIT=bitscan` now restores the old loop in the baseline and compares it with production. `NR_AUDIT=publication` tests removing redundant FP8 encode/decode only for packed-FP8 half GEMMs; that edit is diagnostic only.

Wider CUDA execution is enabled by default: `wideGemm: true` uses eight outputs per thread, paired shared tiles, single-owner packed stores and cached numerical tables. `normalizeAttention: true` folds local normalization into a 32-query attention tile on capable devices. Both retain the previous paths as fallbacks. See [complete implementation and measured results](reports/performance-wide.md). Set either option to false for its previous path; with `npm run benchmark`, use `NR_WIDE=0` / `NR_NORMALIZE=0`. Set `NR_ENHANCED=1` with `npm run benchmark:execution` for the combined off/on comparison. Numerical tables use a separate 258 KiB cache released by cache clearing, workspace resets, or renderer disposal.

The subsequent [exponent/vector optimization](reports/performance-fragments.md) adds aligned FP8 word loads, shared attention exponents, parallel normalization, explicit CUDA loop unrolling and exact four-lane F13 sums. Its attention tile requires 31,680 shared bytes; smaller devices retain the existing fallback.

The [attention reuse pass](reports/performance-reuse.md) extends vector arithmetic to GEMM exponent reductions and paired-channel local attention. It removes per-term integer conversions and reuses probability operands across adjacent output channels.


Tile selection defaults to `gemmTile: 'auto'`. Explicit `'32x32'`, `'64x32'`, and `'32x64'` modes allow profiling on other GPUs (`NR_TILE` with `npm run benchmark` or `npm run benchmark:execution`). Selection checks shared-memory, invocation and X-dimension limits and falls back to the established 32 x 32 tile or the smaller half/scalar path when needed. The policy comes from RTX 5080 measurements; it is not a runtime hardware autotuner.

`npm run benchmark:tiles` measures all three variants at the actual graph row counts using deterministic synthetic data, GPU timestamps, rotating sample order and exact packed-output comparisons. It covers 45 specialization/row combinations and all 358 FP8 GEMM dispatches at each of 720p and 1080p, without requiring a DLL. The report records required and launched workgroups plus weighted isolated timings; these are not frame timings. Use `NR_RESOLUTIONS`, `NR_RUNS`, `NR_TILE_DISPATCHES`, `NR_TILE_FILTER`, and `NR_REPORT` to configure it.

For a paired comparison against an older renderer, `benchmark:execution` accepts `NR_PREVIOUS_GENERATED` for its saved shader directory and `NR_PREVIOUS_ENGINE` for a separately importable saved engine module. Save both before changing dispatch or selection logic; shaders alone do not preserve an older scheduler. The local saved engine must use root-relative imports and `/generated/` URLs so the harness can redirect artifact fetches. These snapshots belong in ignored `reports/*-generated/` directories.

## Experimental model preparation

The [precomputed operands experiment](reports/precomputed-operands.md), `gemmBackend: 'precomputed-half'`, calculates actual scaled weight values and their exponents once, stores them as shader-ready half2 pairs, and reuses them across renders. It removes repeated weight decoding and conversion while preserving the original resolution, vector accumulation and automatic tile policy. Its larger 548.67 MiB representation accepts extra storage to remove arithmetic. All 616 GPU checks, all 75 model boundaries, and the full-resolution output comparisons passed. However, 20 alternating warm samples per backend and resolution found it **1.90% slower at 720p and 1.64% slower at 1080p** on the tested RTX 5080. Standard remains the recommended default. Select **Precomputed operands** in the Model preparation panel, or set `NR_GEMM_BACKEND=precomputed-half`, to compare on other hardware.

The [model preparation pass](reports/model-preparation.md) adds a shared FP8 weight format for `gemmBackend: 'prepared-half'` and `'prepared-integer'`. Preparation rearranges original codes into the shaders' K4 access order and appends exact exponent-bound metadata. The 358 FP8 matrices occupy 145.74 MiB including 8.57 MiB of metadata. Both modes currently use 32 x 32 tiles; the integer mode does not require `shader-f16`. **Standard remains the recommended default.** Both prepared backends passed 492 GPU checks and all 75 model boundaries at 128 x 96, with matching final outputs at 720p and 1080p. In 20 alternating warm samples per backend and resolution on an RTX 5080, prepared half took about 13% longer than Standard and prepared integer took about 3.8 times as long. These complete-backend comparisons include Standard's automatic tile selection versus the prepared kernels' fixed tiles; they do not isolate packing or exponent-metadata costs. See the report for exact timings, sample distributions, and setup costs.

```js
const renderer = await NeuralRenderer.create(model, {
  gemmBackend: 'prepared-half', // or 'prepared-integer'
  modelCache: true,
  onPrepareProgress: console.log
});
```

`prepareModel(model, {cache, onProgress})` is also exported from `src/model-preparation.js`. It returns a map of prepared matrix descriptors and cache statistics. Local IndexedDB keys use actual tensor-content hashes, layout version and matrix identity; saved preparation works across resolutions and page reloads. Records are checked before reuse. Storage failures fall back to CPU preparation. Set `modelCache: false` to disable persistent storage, or call `clearPreparedModelCache()` to clear it. Prepared model bytes remain on the user's device and are never bundled into this project or Pages.

The browser exposes these controls under **Model → Model preparation**. Use `NR_GEMM_BACKEND` with `npm run benchmark`, `npm run test:real`, or `npm run benchmark:execution`. For execution comparisons, `NR_BASELINE_GEMM_BACKEND` selects the baseline (default `half`); other renderer settings are matched. Use `NR_TILE=32x32` to isolate the format/backend change from the production tile policy. The harness records effective dispatch counts and checks final output hashes. `npm run test:model-cache` tests reload, corruption/version recovery and clearing in a browser with GPU access disabled, using synthetic weights unless a local `NR_DLL` is supplied.

...
Prepared execution batches up to 32 graph operations by default, reducing regular benchmark submissions from 84 to 19 per frame. `graphBatchSize` accepts 1�64; smaller batches provide more frequent cancellation opportunities. Streamed execution retains its eight-operation bound and memory-pressure flushes. Cancellation takes effect between submitted batches; it does not interrupt an in-flight GPU dispatch.

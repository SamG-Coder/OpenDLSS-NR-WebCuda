...
This is a **correctness-focused backend**, with 653 graph dispatches per frame and shared-memory tiled attention and shape-selected GEMM for packed FP8 matrices. It emulates Ada's grouped F13/F24 accumulation, preserves half and FP8 publication points, and keeps decoded activations in f32 buffers. Scheduling, packed model caching, and working-buffer reuse reduce overhead; rendering is still not realtime. Large resolutions can exceed the device's buffer limit and are rejected before inference.
...
NR now explicitly requests the adapter's supported storage-buffer and allocation limits instead of WebCuda's conservative 256 MiB default. On the tested RTX 5080 / Edge adapter, this permits buffers up to almost 2 GiB. Real-model benchmarks with the packed cache complete on repeat renders at **1280 × 720** (about 1.01 seconds) and **1920 × 1080** (about 2.34 seconds), with matching source/output dimensions and no rescaling. The working-buffer cache is bounded to 256 MiB and persists between renders at the same resolution; oversized or evicted buffers are destroyed after GPU completion. Actual hardware limits still apply.
...
The current implementation renders the test gradient at **1.01 s for repeat 720p frames** and **2.34 s for repeat 1080p frames**, versus 1.21 s / 2.71 s in the preceding version. Full float32 head and final-output hashes match. First renders populate the model cache and take about 1.49 s / 2.79 s. Measurements exclude model import, shader setup, and UI conversion; see the [attention and tile-selection report](https://github.com/SamG-Coder/OpenDLSS-NR-WebCuda/blob/main/reports/performance-attention.md).

Both attention score and weighted-output calculations use CUDA tiles that share queries, keys, weights, and values while preserving the native reduction order, shifted-window mapping, and padded global attention. `attentionMode: 'scalar'` selects the reference implementation. GEMM variants share one CUDA template, compiled as 4 × 16, 8 × 8, and 8 × 16 tiles. The default `gemmMode: 'auto'` selects by matrix shape using measurements on the reference RTX 5080; it is a fixed heuristic, not runtime hardware autotuning. Explicit modes `tiled` (4 × 16), `tile8x8`, `tile8x16`, and `scalar` remain available for comparisons. The small FP16 projection operations keep the scalar packed kernel.

Working buffers persist in a separate 256 MiB cache with eviction, reset on resolution changes, and release on disposal. `renderer.clearWorkspace()` releases this cache while idle. `NeuralRenderer.create(model, {workspaceCacheBytes: 0})` disables working-buffer caching.

Use `renderer.run({...inputs, profile: true})` to collect per-dispatch GPU timestamps and totals by kernel in `result.profile`. Unsupported devices return an explicit reason instead of estimated GPU timings. Profiling submits individual timestamped passes, so its wall time is not comparable to normal frame timing. Normal runs do not allocate timestamp queries. The result also reports working-buffer allocation/reuse counts.
...
Results are written to ignored `reports/performance.json`. Set `NR_REPORT` to save another report and `NR_COMPARE` to a baseline report to require matching network-head and final-output SHA-256 hashes. Set `NR_PROFILE=1` for GPU timing, `NR_GEMM=scalar|tiled|tile8x8|tile8x16|auto` to select matrix kernels, `NR_ATTENTION=scalar|tiled` to select attention kernels, or `NR_WORKSPACE_MIB=0` to disable workspace caching. `NR_CAPTURE=1` records hashes for all 75 intermediate capture points; when the baseline contains those hashes, `NR_COMPARE` checks them too. Compare on the same hardware and browser, and use profiling-disabled runs for frame-speed comparisons. The benchmark uses a generated gradient and never distributes the DLL or its tensors.

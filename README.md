...
**Performance work parked as of 2026-09-22.** The current browser renderer is the accepted baseline. Compiling this project's CUDA kernels with NVCC produced bit-identical network heads and reduced measured time from about 195 to 158 ms at 720p and 417 to 334 ms at 1080p on the tested RTX 5080; browser wall-clock and native GPU timing scopes differ. The remaining large gap to upstream's tensor-core implementation is not explained by WebCuda translation alone. The exact integer-dot prototype was about 14% slower and remains isolated from production. See the [native CUDA comparison](reports/native-cuda.md) and [integer-dot experiment](reports/exact-integer-dot-proposal.md). Further small tuning gains are not being pursued; revisit for materially better browser matrix-compute capabilities or a demonstrated algorithmic breakthrough. This is a project decision, not a proven hardware ceiling or a measured 1% limit.

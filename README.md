...
The current tiled implementation renders the test gradient at **1.21 s for repeat 720p frames** and **2.71 s for repeat 1080p frames**, versus 1.88 s / 3.70 s in the previous packed scalar version. Full float32 head and final-output hashes match. First renders populate the model cache and take about 1.67 s / 3.30 s. Measurements exclude model import, shader setup, and UI conversion; see the [tiled-kernel report](https://github.com/SamG-Coder/OpenDLSS-NR-WebCuda/blob/main/reports/performance-tiled.md).

The CUDA GEMM shares a 4-row × 16-column tile across 64 threads and preserves native F13/F24 accumulation and publication order. The small FP16 projection operations keep the scalar packed kernel. Working buffers persist in a separate 256 MiB cache with eviction, reset on resolution changes, and release on disposal. `renderer.clearWorkspace()` releases this cache while idle. `NeuralRenderer.create(model, {workspaceCacheBytes: 0})` disables working-buffer caching; `gemmMode: 'scalar'` selects the packed scalar reference for comparisons.
...
Results are written to ignored `reports/performance.json`. Set `NR_REPORT` to save another report and `NR_COMPARE` to a baseline report to require matching network-head and final-output SHA-256 hashes. Set `NR_PROFILE=1` for GPU timing, `NR_GEMM=scalar` to compare the reference kernel, or `NR_WORKSPACE_MIB=0` to disable workspace caching. Compare on the same hardware and browser, and use profiling-disabled runs for frame-speed comparisons. The benchmark uses a generated gradient and never distributes the DLL or its tensors.

...
$('run').onclick=()=>operation(async()=>{
 dimensions({inference:true});const seed=numeric('seed',0,4294967295,true),controls=conditioning(),extra=await extraInputs();if(mode==='object'){scene.configure(sceneSettings());scene.capture($('input'));}
 ...
 const result=await engine.run({width,height,proxy,...extra,seed,conditioning:controls,onProgress:progress,signal:abort.signal});const c=$('output');c.width=width;c.height=height;c.getContext('2d').putImageData(new ImageData(Uint8ClampedArray.from(result.output,v=>Math.round(Math.max(0,Math.min(1,v))*255)),width,height),0,0);
 const blob=await new Promise(r=>c.toBlob(r));if(!blob)throw Error('PNG export failed.');if(downloadURL)URL.revokeObjectURL(downloadURL);downloadURL=URL.createObjectURL(blob);Object.assign($('download'),{href:downloadURL,download:`${sourceName.replace(/\.[^.]+$/,'')||'render'}-nr-${width}x${height}.png`,hidden:false});resultReady=true;$('output-empty').hidden=true;$('output-dims').textContent=`${width} × ${height}`;const seconds=((performance.now()-started)/1000).toFixed(1);status(`Completed in ${seconds} seconds.`);$('result-info').textContent=`${width} × ${height} px · ${seconds} s · Seed ${seed}`;
},{cancel:true});
